        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-5 mt-5 md:px-50'>
            {quiz?.options.map((option , index)=>(
                <h1 key={index} onClick={()=>{
                    ...
                    className={`w-full border border-blue-500 p-3 rounded-2xl text-lg px-5 cursor-pointer hover:bg-blue-700 hover:text-white ${selectedOption == option ? 'bg-blue-700 text-white hover:bg-blue-700' : ''}`}>{option}</h1>
            ))}
        </div>
    </div>
